...
\end{center}
\noindent
This technical documentation provides a complete overview of the PSM Protech Feasibility Study, a project aimed at automating the extraction of geometric tolerances (GD\&T) from mechanical engineering drawings using computer vision, deep learning, and OCR (Optical Character Recognition).

The study addresses the current manual review process of complex technical blueprints, which is time-consuming and prone to human error. The goal is to build a modular, automated system that can accurately identify tolerance symbols, classify their types (e.g., flatness, cylindricity), extract numerical values, and compile them into structured reports—thereby reducing engineering workload and error risk.

The document explains the project’s background, goals, methodology, system architecture, tools, models used, datasets (including synthetic data generation), and constraints. It describes how classification models (YOLOv8-CLS) and OCR pipelines were developed and tested, and summarizes the current results, showing promising accuracy under controlled conditions.

Furthermore, the documentation outlines deployment procedures via Docker, quality assurance results, licensing considerations (e.g., YOLO commercial licensing), and next steps required to scale from synthetic tests to real-world application.

This document is intended for new team members and stakeholders, offering clear guidance through the system’s design, implementation, and future roadmap.
\vfill
\newpage
\tableofcontents
\newpage
