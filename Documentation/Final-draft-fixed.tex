...
\paragraph{Usage:} Once trained, using the model is straightforward. An input image (say, a cropped region from a drawing focusing on a tolerance frame) is passed to the model’s predict function, and it returns the predicted class label (and confidence). For instance, given an image of a symbol like in Figure~Figure~\ref{fig:form_flatness} (left), the model should output “flatness.” 
...
\bibitem{garcia2023} F. Garcia et al., “eDOCr2: Enhanced Document OCR and Retrieval,” \textit{MDPI Journal of Data Science}, vol. 8, 2023.
\bibitem{bakkopi} M. Bakkopi, \texttt{engineering-drawing-extractor}, \url{https://github.com/bakkopi/engineering-drawing-extractor}, accessed May 2025.
